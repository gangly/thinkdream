...
class ArticleModel extends Model {
	...
	public function addData() {
		...
		if (!empty($image_title_alt_word)) {
			// 修改图片默认的title和alt
			$data['content'] = preg_replace('/title=\"(?<=").*?(?=")\"/', 'title="望乡博客"', $data['content']);
			$data['content'] = preg_replace('/alt=\"(?<=").*?(?=")\"/', 'alt="望乡博客"', $data['content']);
		}

		// 将绝对路径转换为相对路径, 必须U，贪婪模式
		$data['content'] = preg_replace('/src=\".*\/Upload\/image\/ueditor/U', 'src="/Upload/image/ueditor', $data['content']);
		// 转义
	...
	public function getPageData($cid = 'all', $tid = 'all', $is_show = '1', $is_delete = 0, $limit = 10) {
		...
		foreach ($list as $k => $v) {
			...
			$v['content'] = preg_replace('/src=\".*\/Upload\/image\/ueditor/U', 'src="' . $root_path . '/Upload/image/ueditor', $v['content']);
	...
	public function getDataByAid($aid, $map = '') {
		...
		if ($map == '') {
			...
			$data['current']['content'] = preg_replace('/src=\".*\/Upload\/image\/ueditor/U', 'src="' . $root_path . '/Upload/image/ueditor', $data['current']['content']);
		}
		return $data;
	}
